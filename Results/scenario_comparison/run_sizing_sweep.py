...
def generate_plots(results_dir, output_dir, input_dir=None):
    ...
    cp5 = ts['CP 5CP Indicator'] if 'CP 5CP Indicator' in ts.columns else None
    cp1 = ts['CP 1CP Indicator'] if 'CP 1CP Indicator' in ts.columns else None
    ...
    if cp5 is not None:
        cp_mask = (cp5 > 0) | (cp1 > 0 if cp1 is not None else False)
    ...
    cp_events = []
    cp_detail_path = os.path.join(results_dir, 'cp_event_detail.csv')
    if os.path.exists(cp_detail_path):
